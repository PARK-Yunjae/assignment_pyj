...
public class ItemDAO {
	...
	public void itemManager(CartDAO cDAO) {
		...
		while (true) {
			System.out.println("[1]아이템 추가"); // 카테고리를 먼저 선택 후 추가 - 같은 카테고리네 아이템 이름 중복 불가
			System.out.println("[2]아이템 수정"); // 카테고리를 먼저 선택 후 아이템 검색
			System.out.println("[3]아이템 삭제"); // 마찬가지로 카테고리를 먼저 선택후 그 안에서 아이템 삭제
			System.out.println("[4]아이템 목록"); // 카테고리기준 전체 아이템 목록 출력
			System.out.println("[0]뒤로가기");
			int sel = InputManager.getInt("선택>>");
			sel = InputManager.selValue(sel, 0, 4);
			if (sel == 1) { // 아이템 추가
				addItem();
			} else if (sel == 2) {// 아이템 수정
				editItem(cDAO);
			} else if (sel == 3) {// 아이템 삭제
				deleteItem(cDAO);
			} else if (sel == 4) {// 아이템 목록
				listItem();
			} else if (sel == 0) {// 뒤로가기
	...
	private void addItem() {
		...
		int itemIdx = categoriItemValue(categori.get(sel), name); // 아이템 중복 확인
		if (itemIdx != -1) {
	...
	private void editItem(CartDAO cDAO) {
		...
		int editItemIdx = categoriItemValue(categori.get(sel), nextName); // 아이템 중복 확인
		if (editItemIdx != -1) {
	...
	private void listItem() {
		for (int i = 0; i < categori.size(); i += 1) {
	...
	public void categoriManager(CartDAO cDAO) {
		while (true) {
			System.out.println("[1]카테고리 추가"); // 1개씩 추가
			System.out.println("[2]카티고리 수정"); // 이름이 바뀌면 장바구니 카테고리도 전부 수정
			System.out.println("[3]카티고리 삭제"); // 삭제 하면 카테고리 목록도 전부 삭제
			...
			if (sel == 1) { // 카테고리 추가
				addCategori();
			} else if (sel == 2) {// 카테고리 수정
				editCategory();
			} else if (sel == 3) {// 카테고리 삭제
				deleteCategori(cDAO);
			} else if (sel == 4) { // 카테고리 목록
				listCategori();
			} else if (sel == 0) {
				System.out.println("관리자 목록으로 돌아 갑니다");
				break;
	...
	private int categoriValue(String categori) {
		for (int i = 0; i < this.categori.size(); i += 1) {
	...
	private void deleteCategori(CartDAO cDAO) {
		...
		deleteCategoryItemList(categori); // 아이템 리스트에 있는 카테고리 전부 삭제
		cDAO.deleteCategory(categori); // 장바구니에 있는 같은 카테고리 목록도 삭제
